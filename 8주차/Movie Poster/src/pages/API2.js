...
export async function fetchMovies(page = 1) {
  ...
  const apiUrl = `https://api.themoviedb.org/3/movie/now_playing?api_key=${apiKey}&page=${page}&language=ko-KR`;
...
export async function fetchNowPlayingMovies(page = 1) {
  const apiKey = '5532b4aedd542db70978e1aface1c6f5';
  const apiUrl = `https://api.themoviedb.org/3/movie/now_playing?api_key=${apiKey}&page=${page}&language=ko-KR`; // 페이지 매개변수를 URL에 포함
...
export async function fetchUpComingMovies() {
  const apiKey = '5532b4aedd542db70978e1aface1c6f5';
  const apiUrl = `https://api.themoviedb.org/3/movie/upcoming?api_key=${apiKey}&language=ko-KR`;
...
export async function fetchSearchMovies(searchTerm) {
  const apiKey = '5532b4aedd542db70978e1aface1c6f5';
  const apiUrl = `https://api.themoviedb.org/3/search/movie?api_key=${apiKey}&query=${searchTerm}&language=ko-KR`;
