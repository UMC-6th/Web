...
function NavBar() {
  ...
  return (
    <Wrap>
      <Link className='navmenu' to={'/'}><Menu>UMC Movie</Menu></Link>
      <Pages>
        {auth.isAuthenticated ? (
          <Menu onClick={logout}>로그아웃</Menu>
        ) : (
          <Link className='navmenu' to={'/login'}><Menu>로그인</Menu></Link>
        )}
        <Link className='navmenu' to={'/signup'}><Menu>회원가입</Menu></Link>
